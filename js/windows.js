...
const Windows = {
  ...
  toggleHelp() {
    ...
    // physical keys, so the labels have to come from the real layout
    const kb = (code) => '<kbd>' + escapeHtml(KeysPanel.keyLabel(code)) + '</kbd>';
    const rows = [
      ['<kbd>Space</kbd>', tr('help_play', 'Play or pause')],
      ['<kbd>Enter</kbd>', tr('help_stop', 'Stop')],
      [kb('KeyR'), tr('help_record', 'Record')],
      [kb('KeyM'), tr('help_metronome', 'Metronome')],
      [kb('KeyK'), tr('help_keyboard', 'Keyboard')],
      [kb('KeyX'), tr('help_mixer', 'Mixer')],
      ['<kbd>' + tr('help_col_arrows', 'Arrows') + '</kbd>', tr('help_arrows', 'Move the selected notes or patterns')],
      ['<kbd>Shift</kbd><kbd>' + tr('help_col_arrows', 'Arrows') + '</kbd>', tr('help_arrows_big', 'Move by a bar, or by an octave')],
      ['<kbd>Alt</kbd><kbd>' + tr('help_col_leftright', 'Left') + '</kbd><kbd>' + tr('help_col_right', 'Right') + '</kbd>', tr('help_arrows_len', 'Make the selected notes longer or shorter')],
      ['<kbd>Alt</kbd><kbd>' + tr('help_col_updown', 'Up') + '</kbd><kbd>' + tr('help_col_down', 'Down') + '</kbd>', tr('help_arrows_vel', 'Make the selected notes louder or quieter')],
      [tr('help_col_arrows_none', 'Arrows with nothing selected'), tr('help_arrows_playhead', 'Move the playhead')],
      ['<kbd>Home</kbd><kbd>End</kbd>', tr('help_home_end', 'Playhead to the start or the end')],
      ['<kbd>Cmd</kbd><kbd>Z</kbd>', tr('help_undo', 'Undo')],
      ['<kbd>Cmd</kbd><kbd>Y</kbd>', tr('help_redo', 'Redo')],
      ['<kbd>Cmd</kbd><kbd>A</kbd>', tr('help_selectall', 'Select every note, or every pattern')],
      ['<kbd>Cmd</kbd><kbd>C</kbd> <kbd>X</kbd> <kbd>V</kbd>', tr('help_copy', 'Copy, cut, paste')],
      ['<kbd>Cmd</kbd><kbd>D</kbd>', tr('help_duplicate', 'Duplicate, right after the selection')],
      ...
      [Object.keys(WHITE_CODES).slice(0, 4).map(kb).join(''), tr('help_white_keys', 'White keys')],
      [Object.keys(BLACK_CODES).slice(0, 5).map(kb).join(''), tr('help_black_keys', 'Black keys')],
      [kb('KeyZ') + ' ' + kb('KeyX'), tr('help_octave', 'Octave down, up')],
